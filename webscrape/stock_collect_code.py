    ...
    try:
        ...
        if "EQUITY" in json_loaded_context["_context"]["quoteType"]:
            ...
            day_range = json_loaded_summary["QuoteSummaryStore"]["summaryDetail"]["regularMarketDayLow"]['fmt'] \
                        + " - " + json_loaded_summary["QuoteSummaryStore"]["summaryDetail"]["regularMarketDayHigh"][
                            'fmt']
            ...
            summary_data.update(
                {'Previous Close': prev_close, 'Open': mark_open, 'Bid': bid, 'Ask': ask, "Day's Range": day_range \
                    , '52 Week Range': year_range, 'Volume': volume, 'Avg. Volume': avg_volume,
                 'Market Cap': market_cap, 'Beta (3Y Monthly)': beta \
                    , 'PE Ratio (TTM)': PE, 'EPS (TTM)': eps, 'Earnings Date': earnings_date,
                 'Forward Dividend & Yield': div \
            ...
            exp_rat = json_loaded_summary["QuoteSummaryStore"]["defaultKeyStatistics"]["annualReportExpenseRatio"][
                'fmt']
            ...
            morningstar_rating = \
                json_loaded_summary["QuoteSummaryStore"]["defaultKeyStatistics"]["morningStarOverallRating"]['raw']
            morningstar_risk_rating = \
                json_loaded_summary["QuoteSummaryStore"]["defaultKeyStatistics"]["morningStarRiskRating"]['raw']
            ...
            five_year_avg_ret = \
                json_loaded_summary["QuoteSummaryStore"]["fundPerformance"]["performanceOverview"][
                    "fiveYrAvgReturnPct"][
                    'fmt']
            holdings_turnover = \
                json_loaded_summary["QuoteSummaryStore"]["defaultKeyStatistics"]["annualHoldingsTurnover"]['fmt']
            div = json_loaded_summary["QuoteSummaryStore"]["defaultKeyStatistics"]["lastDividendValue"]['fmt']
            inception_date = json_loaded_summary["QuoteSummaryStore"]["defaultKeyStatistics"]["fundInceptionDate"][
                'fmt']

            # Store ordered pairs to be written to a file
            summary_data.update({'Previous Close': prev_close, 'YTD Return': ytd_return, 'Expense Ratio (net)': exp_rat,
                                 'Category': category \
                                    , 'Last Cap Gain': last_cap_gain, 'Morningstar Rating': morningstar_rating,
                                 'Morningstar Risk Rating': morningstar_risk_rating \
                                    , 'Sustainability Rating': sustainability_rating, 'Net Assets': net_assets,
                                 'Beta (3Y Monthly)': beta, 'Yield': yld \
                                    , '5y Average Return': five_year_avg_ret, 'Holdings Turnover': holdings_turnover,
                                 'Last Dividend': div, 'Average for Category': 'N/A' \
            ...
            beta = json_loaded_summary["QuoteSummaryStore"]["defaultKeyStatistics"]['beta3Year']['fmt']
            exp_rat = json_loaded_summary["QuoteSummaryStore"]["fundProfile"]["feesExpensesInvestment"][
            ...
            summary_data.update(
                {'Previous Close': prev_close, 'Open': mark_open, 'Bid': bid, 'Ask': ask, "Day's Range": day_range,
                 '52 Week Range': year_range \
                    , 'Volume': volume, 'Avg. Volume': avg_volume, 'Net Assets': net_assets, 'NAV': nav,
                 'PE Ratio (TTM)': 'N/A', 'Yield': yld, 'YTD Return': ytd_return \
                    , 'Beta (3Y Monthly)': beta, 'Expense Ratio (net)': exp_rat, 'Inception Date': inception_date,
...
if __name__ == "__main__":
    ...
    # tickers = ["abt","msft","aapl","afl","apc"]

    count = 0
    # "KR", "MTG", "R", "CVS", "IFF", "DUK", "KMB", "TMO", "BMS",
    # tickers = ["ANDW","SUN","A","GLK","HNZ","BBT","PDG","ED","AHP","XOM","P","ABT","MSFT",
    #            "AAPL","AFL","EPG","BLL","LTR","PWER","GPC","BCR","AOC","CPB","HON","HI","BEN","BK","INTC","QCOM","RBK","KSS","LUV","PEG",
    #            "CINF","ATI","ZMH","BOL","K","SYY","GTW","IBM","S","WFC","TMK",
    #            "BLS","MDR","K","CAH","ADM","GIS","KRB","UN","HLT","CTXS","DDS","PHM","NSI","PFE","KBH","SFA","YHOO","S","SBC","NUE","UIS",
    #             "AMGN","YUM","WMT","DCN","ECL","SLM","OMC","CB","UNH","W","V","COL","AM","BGEN","PCAR","AT","MWD","FMC","EFX","ABI","AWE","AES"]
    ...
    for ticker in tickers:
        count += 1
        print('count: ticker', ticker, count)
