...
function updateChart() {
    ...
    function tick(e) {
        dots.each(moveTowardDataPosition(e.alpha));

        if (checkbox.node().checked) dots.each(collide(e.alpha));
    ...
    function moveTowardDataPosition(alpha) {
        return function(d) {
            d.x += (x(d[chartScales.x]) - d.x) * 0.1 * alpha;
            d.y += (y(d[chartScales.y]) - d.y) * 0.1 * alpha;
        };
    }
